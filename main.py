...
# Стили оставляем те же (твоя имба-дизайн база)
STYLE = '''
<style>
    ...
    .top-bar { display: flex; align-items: center; justify-content: center; position: relative; margin-bottom: 25px; }
    ...
    .heart-btn { cursor: pointer; font-size: 2rem; color: #333; transition: 0.3s; background: none; border: none; }
...
def get_sidebar(active_page, file_url=None):
    # Добавляем атрибут download для подстраховки
...
def home():
    return render_template_string(f'''
        ...
        <body>{get_sidebar('home')}<div class="main"><h1>Главное меню</h1>
            <div class="grid">
                <a href="/wurst" class="card"><h3>Wurst</h3><p>Удобный клиент для выживания.</p><div class="card-footer"><span class="card-version">1.21.11</span></div></a>
                <a href="/meteor" class="card"><h3>Meteor Client</h3><p>Стабильная сборка для PVP.</p><div class="card-footer"><span class="card-version">1.21.11</span></div></a>
            </div>
...
@app.route('/wurst')
def wurst_page():
    # Используем githack для прямого скачивания
    file_link = "https://raw.githack.com/r1ze-r/HK/main/Wurst-Client1.21.11-hk.jar"
    return render_template_string(f'''
        ...
        <body onload="loadHeartState('wurst')">{get_sidebar('wurst', file_link)}
        <div class="main"><div class="top-bar"><a href="/" class="btn-back-abs">← Назад</a>
        <div class="heart-container"><button id="heart-wurst" class="heart-btn" onclick="toggleLike('wurst', 'Wurst')">❤</button></div>
        </div><h1>Wurst Client 1.21.11-hk</h1><p>Легендарная классика для Minecraft.</p></div>{SCRIPTS}</body></html>
    ''')

@app.route('/meteor')
def meteor_page():
    # Используем githack для прямого скачивания
    file_link = "https://raw.githack.com/r1ze-r/HK/main/meteor-client-1.21.11-hk.jar"
    return render_template_string(f'''
        ...
        <body onload="loadHeartState('meteor')">{get_sidebar('meteor', file_link)}
        <div class="main"><div class="top-bar"><a href="/" class="btn-back-abs">← Назад</a>
        <div class="heart-container"><button id="heart-meteor" class="heart-btn" onclick="toggleLike('meteor', 'Meteor Client')">❤</button></div>
        </div><h1>Meteor Client 1.21.11-hk</h1><p>Лучший выбор для PVP потасовок.</p></div>{SCRIPTS}</body></html>
    ''')
